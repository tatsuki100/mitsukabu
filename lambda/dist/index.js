...
// convertData.ts
var convertData_exports = {};
__export(convertData_exports, {
  convertYahooToDailyData: () => convertYahooToDailyData,
  filterNullData: () => filterNullData
});
var filterNullData, convertYahooToDailyData;
var init_convertData = __esm({
  "convertData.ts"() {
    "use strict";
    filterNullData = (timestamps, open, high, low, close, volume) => {
      const nullDates = [];
      const validIndices = [];
      timestamps.forEach((timestamp, index) => {
        const hasNull = open[index] === null || high[index] === null || low[index] === null || close[index] === null || volume[index] === null;
        if (hasNull) {
      ...
      const filteredOpen = validIndices.map((i) => open[i]);
      const filteredHigh = validIndices.map((i) => high[i]);
      const filteredLow = validIndices.map((i) => low[i]);
      const filteredClose = validIndices.map((i) => close[i]);
      const filteredVolume = validIndices.map((i) => volume[i]);
      return {
        filteredData: {
    ...
    convertYahooToDailyData = (yahooResponse) => {
...
// fetchYahooFinance.ts
init_convertData();
var generateYahooFinanceURL = (stockCode) => {
  return `https://query1.finance.yahoo.com/v8/finance/chart/${stockCode}.T?interval=1d&range=7mo`;
};
var fetchSingleStock = async (stockCode, stockName, maxRetries = 3) => {
...
// database.ts
var saveAllStocksToDb = async (results) => {
  ...
  await sql.transaction(
    results.map(
      (result) => sql`INSERT INTO stock_data (stock_code, stock_name, daily_data, last_update)
...
var callRevalidateEndpoint = async () => {
...
var handler = async () => {
  ...
  try {
    ...
    const parseResult = import_papaparse.default.parse(csvContent, {
      header: true,
      skipEmptyLines: true
    });
    ...
    const successResults = allResults.filter(
      (r) => r.success && r.dailyData !== void 0
    ).map((r) => ({
...
if (process.env.LOCAL_EXEC) {
  Promise.resolve().then(() => (init_convertData(), convertData_exports)).then(() => {
    handler().then((result) => {
