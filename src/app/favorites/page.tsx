...
const FavoritesPage = () => {
  ...
  // ページネーション計算
  const totalItems = favoriteStocks.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  const startIndex = currentPage * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, totalItems);
  const displayStocks = favoriteStocks.slice(startIndex, endIndex);
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-gray-100 border border-gray-300 text-gray-700 px-6 py-8 rounded-lg text-center">
          ...
          <h3 className="text-xl font-bold mb-2">観察銘柄がありません</h3>
          <p className="mb-4">銘柄一覧ページで星アイコンをクリックして、観察銘柄を追加してください。</p>
          <button
            onClick={() => router.push('/')}
            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
          >
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {displayStocks.map((stock) => (
          <StockCard key={stock.code} stock={stock} linkPrefix="favorites" />
        ))}
      </div>
