...
const CrossVPage = () => {
  ...
  // スクリーニング機能
  const { crossVStocks, crossVCount } = useStockScreening();
  ...
  // データが変わった時にページをリセット（銘柄数が変わった場合のみ）
  useEffect(() => {
    const currentStockCount = crossVStocks.length;
    
    // 初回ロードまたは銘柄数が実際に変わった場合のみリセット
    if (prevStockCountRef.current !== currentStockCount) {
      console.log(`クロスV銘柄数変更: ${prevStockCountRef.current} → ${currentStockCount}`);
      setCurrentPage(0);
      prevStockCountRef.current = currentStockCount;
    }
  }, [crossVStocks.length]); // lengthのみを依存関係にする

  // ページネーション計算
  const totalItems = crossVStocks.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  const startIndex = currentPage * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, totalItems);
  const displayStocks = crossVStocks.slice(startIndex, endIndex);

  // ページ変更ハンドラー
  const handlePageChange = useCallback((newPage: number) => {
    console.log(`クロスVページ変更: ${currentPage} → ${newPage}`);
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-gray-100 border border-gray-300 text-gray-700 px-6 py-8 rounded-lg text-center">
          ...
          <h3 className="text-xl font-bold mb-2">クロスV銘柄がありません</h3>
          {/* <p className="mb-4">現在、クロスV条件に該当する銘柄はありません。</p> */}
          <div className="mt-4 text-sm text-gray-600">
            <p>【クロスV条件】</p>
            <p>・短期移動平均線（5日）が中期移動平均線（25日）または長期移動平均線（75日）の下にある</p>
          </div>
          <button
            onClick={() => router.push('/')}
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {displayStocks.map((stock) => (
          <StockCard key={stock.code} stock={stock} linkPrefix="cross_v" />
        ))}
      </div>
