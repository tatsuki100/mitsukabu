...
const ScreeningPage = () => {
  ...
  useEffect(() => {
    const currentStockCount = turnbackStocks.length;
    
    // 初回ロードまたは銘柄数が実際に変わった場合のみリセット
    if (prevStockCountRef.current !== currentStockCount) {
      console.log(`銘柄数変更: ${prevStockCountRef.current} → ${currentStockCount}`);
      setCurrentPage(0);
      prevStockCountRef.current = currentStockCount;
    }
  }, [turnbackStocks.length]); // lengthのみを依存関係にする

  // ページネーション計算
  const totalItems = turnbackStocks.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  const startIndex = currentPage * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, totalItems);
  const displayStocks = turnbackStocks.slice(startIndex, endIndex);
